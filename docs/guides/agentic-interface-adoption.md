...
| Project Mgr | `projectmgr` | Land a stated unit of work with exact state and GO |
...
- End the commit message and the PR body with `Role`, `Co-authored-by`, then
  `Committer-of-Record`, after one blank line. See
  [attribution-footer.md](../repository/attribution-footer.md).
- Use `noreply@` one of `3leaps.net`, `3leaps.dev`, `fulmenhq.dev`,
  `lanytehq.dev` for the Co-authored-by email
- Use a bare `Role:` slug (`devlead`, not `agent-devlead` or `bravo-devlead`)
- `Committer-of-Record:` is `@3leapsdave`
...
- [Attribution Footer](../repository/attribution-footer.md)

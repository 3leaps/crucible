...
| [`projectmgr`](../../../config/agentic/roles/projectmgr.yaml) | Chairing standing program/panel threads: status, next actions, owners, stuck detection                             | Keep distinct from `dispatch`, `deliverylead`, and `cxotech`; treat Luna Max as a candidate pin, not an equal default |
